...
<sub>Detects and auto-configures Claude Code, Cursor, Codex CLI, opencode, Hermes Agent, Gemini CLI, Antigravity IDE, Kiro, Pi, and GitHub Copilot (VS Code, Copilot CLI, JetBrains IDEs) — wiring CodeGraph into each. Pi also receives deterministic prompt front-loading, so local models get relevant graph context before generation without needing to choose a tool first. **This is the step that connects CodeGraph to your agent;** installing the CLI in step 1 does not do it on its own. It only wires up your agent — it does **not** index any code; building each project's graph is the separate `codegraph init` in step 3. (Shortcut: `npx @colbymchenry/codegraph` downloads and runs this in one go.)</sub>
...
The installer will:
- Ask which agent(s) to configure — auto-detects installed ones from: **Claude Code**, **Cursor**, **Codex CLI**, **opencode**, **Hermes Agent**, **Gemini CLI**, **Antigravity IDE**, **Kiro**, **Pi**, **GitHub Copilot** (VS Code, Copilot CLI, JetBrains IDEs)
...
Restart your agent (Claude Code / Cursor / Codex CLI / opencode / Hermes Agent / Gemini CLI / Antigravity IDE / Kiro / Pi / VS Code, the Copilot CLI, or your JetBrains IDE for GitHub Copilot) for CodeGraph to load. Pi can alternatively run `/reload`.
...
**Made for AI coding agents — Claude Code, Cursor, Codex CLI, opencode, Hermes Agent, Gemini CLI, Antigravity IDE, Kiro, Pi, and GitHub Copilot**
